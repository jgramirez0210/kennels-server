    ...
    with sqlite3.connect("./kennel.sqlite3") as conn:
        ...
        for row in data:
            if row['employee_id'] is not None:
                ...
                location.employee.append(employee.__dict__)  # Add the employee to the employees list

            if row['animal_id'] is not None:
                animal = Animal(row['animal_id'], row['animal_name'], row['animal_breed'], row['animal_status'])
                location.animal.append(animal.__dict__)  # Add the animal to the animals list
